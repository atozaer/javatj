...
public class ProductController extends AdminMainController {
	ProductRepository productRepository = new ProductRepository();
	ProductView productView = new ProductView();
	view.ProductView userProductView;
	List<ProductVO> productList;
	ProductVO product;
	
	@Override
	public void play() {
		switch (productView.menuSelector()) {
			case "i" :
				Function.productCrolling();
				break;
			case "l":
				userProductView = new view.ProductView();
				userProductView.list(productRepository.selectAll());
				break;
			case "c":
			...
			case "m":
				...
				if (modifyResult.equals("y")) {
					...
					productView.modifyResult(productRepository.update(product));
				}
				break;
			case "d":
				...
				if (deleteResult.equals("y")) {
					productView.deleteResult(productRepository.deleteById(deleteTargetNo));
				}
				break;
			case "s":
				productList = productRepository.selectFindById(
				...
				if (stockAddResult.equals("y")) {
					productView.modifyResult(productRepository.update(productView.stockAdd(productList.get(0))));
				}
				break;
			default :
